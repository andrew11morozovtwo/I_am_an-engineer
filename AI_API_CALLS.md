...
- **⚠️ ОПТИМИЗАЦИЯ:** В `channel_router.py` результат `prepare_message_content` сохраняется и переиспользуется, чтобы избежать дублирования API-вызовов
...
- **Файл:** `app/presentation/routers/channel_router.py`
  ...
  - **Строка 648:** Подготовка контента комментария пользователя для генерации ответа (использует уже подготовленный контент из проверки blacklist)
  - **Строка 688:** Подготовка контента оригинального поста для контекста

**⚠️ ОПТИМИЗАЦИЯ (реализовано):**
- В `discussion_message_handler` результат `prepare_message_content` сохраняется в переменную `full_content` при проверке blacklist
- При генерации AI-ответа используется уже подготовленный контент, что исключает дублирование API-вызовов
- Это значительно снижает количество запросов к OpenAI Vision API и Whisper API
